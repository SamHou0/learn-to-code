---
title: const 限定符与指针
description: 详细讲解 C++ 中 const 限定符与指针的结合使用，包括常量引用、常量指针的定义规则，以及顶层 const 和底层 const 的区别与应用场景。
---
...
**`const` 应用在引用上时，只是让 C++ ***认为***引用指向的对象不可以被修改，而实际指向的对象到底是否为常量（是否可以修改）是没有影响的。**
...
我们曾经在[特殊用法](#特殊用法)那里提过一嘴自动转换。众所周知，在执行赋值操作时，可能会进行自动转换。而变量可以转换为常量，常量也可以转换为变量：
