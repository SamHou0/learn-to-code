---
title: 数组与指针
description: 本文以浅显生动的方式讲解了C++中数组与指针的关系，展示了数组的遍历方式、多维数组的本质及范围for的应用，让读者理解数组退化为指针的机制与多维结构的内在逻辑。
---

在[上一篇](https://blog.samhou.moe/cpp-vector-iterator-guide/)中，我们讨论了 vector 和迭代器，用以遍历一个有序可变序列。而我们知道，在 vector 之下有一种更加基本的数据类型——数组，它是有序**固定大小**的序列。实际上，我们所涉及的迭代器（范围 for），在数组中也以某种形式可用。
...
没错，在[指针](https://blog.samhou.moe/cpp-pointer-guide/)那一篇里，我们曾经见过。这是十六进制数，表示一个**地址**。也就是说，我们输出了数组，但是它的表现居然和指针一样，输出了一个对象的地址！
...
![数组指针的实际工作原理](https://img.samhou.top/1762567795149.png)
...
是不是很熟悉？我们曾经提到过，[范围 for 语句](https://blog.samhou.moe/cpp-vector-iterator-guide/)满足的条件：
...
这样我们就不难理解，第一个下标决定是哪一个*包含 10 个元素数组*，第二个下标决定获取到的数组中的哪一个元素。
...
但是等等，让我们回忆一下之前的[范围 for 的介绍](https://blog.samhou.moe/cpp-vector-iterator-guide/)。我们提到过，如果想要修改，可以创建引用，那么就不会拷贝赋值了。
