---
title: 左值和右值
description: 用通俗例子讲清 C++ 左值/右值、引用与const、取址和解引用、下标、前后置自增、范围for的拷贝与引用差异，纠正常量指针示例与误区，强调左值可读写、右值多为临时结果。
---
...
之前在 [const 限定符](https://blog.samhou.moe/cpp-pointer-const-guide/)一节，已经介绍了常量、常量指针和常量引用。那么它们是左值还是右值呢？
...
我们之前提到过[范围 for](https://blog.samhou.moe/cpp-vector-iterator-guide/) 的拷贝赋值问题，现在再用左值和右值的视角回顾一下这个问题（拿出一样的代码）。冒号前面的变量是否为引用，决定了是左值还是右值：
